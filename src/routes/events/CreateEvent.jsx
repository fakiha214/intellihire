...
const CreateEvent = () => {
  ...
  useEffect(() => {
    // Load pricing so we can show the hosting fee on the review step
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!eventId) {
        // New event was created as an unpaid draft. Send the host to Stripe
        // Checkout to pay the one-time hosting fee and activate it.
        try {
          const coRes = await fetch(`/api/events/${data.id}/checkout`, {
            method: 'POST',
            credentials: 'include'
          })
        ...
        } catch (_e) { /* fall through to dashboard, payment can be completed there */ }
        setTimeout(() => navigate(`/events/${data.id}/dashboard`), 800)
      } else {
        setTimeout(() => navigate(`/events/${eventId}/dashboard`), 1200)
      }
    } catch (_err) { // eslint-disable-line no-unused-vars
              ...
              {!eventId && (
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm font-semibold text-blue-900">
                    One-time hosting fee{paymentInfo ? `: ${paymentInfo.amountDisplay}` : ''}
                  </p>
                  <p className="text-xs text-blue-800 mt-1">
                    After creating the event you'll be redirected to secure Stripe checkout to activate it.
                    The event stays a private draft until payment is complete.
                  </p>
                </div>
              )}
            </div>
